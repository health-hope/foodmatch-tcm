<template>
    <div class="crad-wrap">
        <div v-transfer-dom>
            <x-dialog v-model="selectTypePop" hide-on-blur>
                <div class="pop1-select-btn" @click="no">
                    <LinkTop></LinkTop>
                    <ul class="select-btn">
                        <li
                            :class="[item.value.length>15?'radius2':'radius1',index==typeIndex?'active':'']"
        ...
        <div v-transfer-dom class="timeWrap">
            <x-dialog v-model="selectTimePop" hide-on-blur>
                <div class="pop1-select-btn font-big" @click="no">
                    <LinkTop></LinkTop>
                    <datetime-view
                        :start-date="startDay"
                        :end-date="endDay"
                        ...
                        :format="format"
                    ></datetime-view>
                    <div class="btn-commit" @click="btnTime" :style="{backgroundImage: 'url(sy_button_bg.png)'}  | PUBLIC_PATH">确定</div>
                    <LinkBottom></LinkBottom>
                </div>
            </x-dialog>
        </div>
        <div v-transfer-dom>
            <x-dialog v-model="selectSexPop" hide-on-blur>
                <div class="pop1-select-btn" @click="no">
                    <LinkTop></LinkTop>
                    <div class="sex-box">
                        <div
                            :class="[item.className,index==basicInfo.sex-1?'active':'']"
                            v-for="(item,index) in sexArr"
                            :key="index"
                            @click="selectSexHandle(index)"
                        >
                            <div class="avatar" :style="{backgroundImage: 'url(cpy_icon_nan.png)'}  | PUBLIC_PATH" v-if="item.className=='sex-boy'"></div>
                            <div class="avatar" :style="{backgroundImage: 'url(cpy_icon_nv.png)'}  | PUBLIC_PATH" v-if="item.className=='sex-women'"></div>
                        </div>
                    </div>
                    <LinkBottom></LinkBottom>
                </div>
        ...
        <div v-transfer-dom class="heightWrap">
            <x-dialog v-model="selectHeightPop" hide-on-blur>
                <div class="pop1-select-btn font-big" @click="no">
                    <LinkTop></LinkTop>
                    <picker :data="heightArr" v-model="selectHeightNum" @on-change="changeHeight"></picker>
                    <div class="btn-commit" :style="{backgroundImage: 'url(sy_button_bg.png)'}  | PUBLIC_PATH"  @click="selectHeight">确定</div>
                    <LinkBottom></LinkBottom>
                </div>
            </x-dialog>
        </div>
        <div v-transfer-dom class="weightWrap">
            <x-dialog v-model="selectWeightPop" hide-on-blur>
                <div class="pop1-select-btn font-big " @click="no">
                    <LinkTop></LinkTop>
                    <picker :data="weightArr" v-model="selectWeightNum" @on-change="changeWeight"></picker>
                    <div class="btn-commit"  :style="{backgroundImage: 'url(sy_button_bg.png)'}  | PUBLIC_PATH" @click="selectWeight">确定</div>
                    <LinkBottom></LinkBottom>
                </div>
            </x-dialog>
        </div>
        <div v-transfer-dom>
            <x-dialog v-model="selectLevelPop" hide-on-blur>
                <div class="pop1-select-btn" @click="no">
                    <LinkTop></LinkTop>
                    <ul class="select-btn">
                        <li
                            :class="[item.length>15?'radius2':'radius1', index==levelIndex?'active':'']"
        ...
        <div v-transfer-dom>
            <x-dialog v-model="selectTastePop" hide-on-blur>
                <div class="pop1-select-btn" @click="no">
                    <LinkTop></LinkTop>
                    <ul class="select-btn">
                        <li
                            :class="[item.length>15?'radius2':'radius1', index+1==tasteIndex?'active':'']"
        ...
        <div v-transfer-dom>
            <x-dialog v-model="selectCityPop" hide-on-blur>
                <div class="pop1-select-btn font-big" @click="no">
                    <LinkTop></LinkTop>
                    <picker :data="selectCity" :fixed-columns="2" :columns="3" v-model="initCity" @on-change="changeDietRecommend"></picker>
                    <div class="btn-commit" :style="{backgroundImage: 'url(sy_button_bg.png)'}  | PUBLIC_PATH" @click="selectCityBtn">确定</div>
                    <LinkBottom></LinkBottom>
                </div>
            </x-dialog>
        </div>
        <div v-transfer-dom>
            <x-dialog v-model="selectExtraMeal" hide-on-blur @on-hide="clasePop($event)">
                <div class="pop1-select-btn" @click="no">
                    <div class="icon-text">多选</div>
                    <LinkTop></LinkTop>
                    <ul class="select-btn select-btn-add">
                        <li
                        ...
                        >
                            <div
                                class="btn"
                                :class="[item.length>15?'radius2':'radius1',index<=2?'active':'',index<=2?'mix-fonts':'' ,dietRecommend.extraMealList.indexOf(`${index+1}`)!=-1?'active':'','mix-height-li']"
                            >{{item}}</div>
                            <div class="tit must-text" v-if="index==2">
                    ...
                    <div class="btn-commit"  :style="{backgroundImage: 'url(sy_button_bg.png)'}  | PUBLIC_PATH" @click="clasePopBtn($event)">确定</div>
                    <LinkBottom></LinkBottom>
                </div>
            </x-dialog>
        </div>
        ...
        <div class="crad-item">
            <BaseZyTempTitle :bgClass="bgClass1" :tempTitle="tit"></BaseZyTempTitle>
            <div class="crad-content">
                <LinkTop></LinkTop>
                <div class="content">
                    <div class="item-link" @click="showPops1('type',nameType)">
        ...
        <div class="crad-item">
            <BaseZyTempTitle :bgClass="bgClass2" :tempTitle="tempTitle2"></BaseZyTempTitle>
            <div class="crad-content">
                <LinkTop></LinkTop>
                <div class="content">
                    <div class="item-link" @click="showPops1('time',basicInfo.birthday)">
                        <div class="select-name">
                            <span>出生日期</span>
                        </div>
                        <div class="select-box">
                    ...
                    <div class="item-link" @click="showPops1('sex',basicInfo.sex)">
                        <div class="select-name">
                            <span>性别</span>
                        </div>
                        <div class="select-box">
                            <div
                                class="select-font"
                            >{{basicInfo.sex?toggleSex(basicInfo.sex):"请选择"}}</div>
                            <span
                                class="jt"
                                :style="{backgroundImage: 'url(pctj_icon_jiantou.png)'}  | PUBLIC_PATH"
                            ></span>
                    ...
                    <div class="item-link" @click="showPops1('height',basicInfo.height)">
                        <div class="select-name">
                            <span>身高</span>
                        </div>
                        <div class="select-box">
                            <div class="select-font">{{basicInfo.height?basicInfo.height+"厘米":"请选择"}}</div>
                            <span
                                class="jt"
                                :style="{backgroundImage: 'url(pctj_icon_jiantou.png)'}  | PUBLIC_PATH"
                            ></span>
                    ...
                    <div class="item-link" @click="showPops1('weight',basicInfo.weight)">
                        <div class="select-name">
                            <span>体重</span>
                        </div>
                        <div class="select-box">
                    ...
                    <div class="item-link" @click="showPops1('level',basicInfo.weight)">
                        <div class="select-name">
                            <span>劳动强度</span>
                        </div>
                        <div class="select-box">
        ...
        <div class="crad-item">
            <BaseZyTempTitle :bgClass="bgClass3" :tempTitle="tempTitle3"></BaseZyTempTitle>
            <div class="crad-content">
                <LinkTop></LinkTop>
                <div class="content">
                    <div class="item-link" @click="showPops1('taste',basicInfo.weight)">
                        <div class="select-name">
                            <span>口味</span>
                    ...
                    <div class="item-link" @click="showPops1('city',basicInfo.weight)">
                        <div class="select-name">
                            <span>地区</span>
                            <span class="optional">（选填）</span>
                        </div>
                        <div class="select-box">
                            <div class="select-font">{{selectCityText?selectCityText:"请选择"}}</div>
                            <span
                                class="jt"
                                :style="{backgroundImage: 'url(pctj_icon_jiantou.png)'}  | PUBLIC_PATH"
                            ></span>
...
export default {
    data() {
        return {
            bgClass1: "jgy_bg_xxzhcp",
            tit: "",
            bgClass2: "jgy_bg_wsmhdzxyx",
            tempTitle2: "身体基本信息",
            bgClass3: "jgy_bg_smsxyx",
            tempTitle3: "饮食偏好信息",
            ...
            selectExtraMealOld: [], //之前選擇的
            selectCityPop: false, //城市弹框
            selectCity: [], //选择省市
            initCity: [], //初始化的省市
            address: "", //地区 省+市
            province: "", //省 存放已选数据，为最终数据
            city: "", //市 存放已选数据，为最终数据
            selectCityText: ""
        };
    },
    ...
    mounted() {
        getUserInfo(this.code, this.userId, config.isAssociated).then(res => {
            ...
            if (res.dietRecommend) {
                this.dietRecommend = res.dietRecommend;
                this.tasteIndex = res.dietRecommend.taste;
                this.dietRecommend.extraMealList = Array.from(new Set([...this.dietRecommend.extraMealList, "1", "2", "3"]))
                this.initCity = [
                    res.dietRecommend.province,
                    res.dietRecommend.city
                ];
                this.selectCityText = res.dietRecommend.province + res.dietRecommend.city;
                // 初始化数据为已选中的默认数据
                this.city = res.dietRecommend.city || '';
                this.province = res.dietRecommend.province || '';
            }else{
                // 兼容后台返回多余的字段[0]
                this.dietRecommend.extraMealList = [
                    "1",
                    "2",
                    "3"
                ]
            ...
            console.log(this.weightArr, "this.heightArrweight ");
        });
    },
    methods: {
        // 单项滑动弹框显示隐藏
        showPops1(types, text) {
            ...
            if (types == "type") {
                ...
                // 初始化数据为当前已选中或者默认第一条数据
                this.initCity = [this.city,this.province] || this.selectCity[0];
                ...
                console.log(this.selectExtraMealOld, "zhiqianxuande");
            }
        },
        // 点击哪个类型
        selectType(e) {
        ...
        selectSexHandle(index) {
            ...
            switch (index) {
                case 0:
                    return (this.basicInfo.sex = "1");
                    breack;
                case 1:
                    return (this.basicInfo.sex = "2");
                    breack;
            }
        },
        toggleSex(n) {
        ...
        changeDietRecommend(value) {
            // this.dietRecommend 中的city和province为临时数据，存储起来
            this.dietRecommend.city = value[0];
            this.dietRecommend.province = value[1];
        },
        selectCityBtn() {
            // this.city this.province 为最终数据
            this.city = this.dietRecommend.city;
            this.province = this.dietRecommend.province;
            this.selectCityText = this.city + this.province;
        ...
        commit() {
            ...
            postUpsert(code, userId, resultType, basicInfo, dietRecommend).then(
                res => {
...
.crad-wrap {
    ...
    .btn {
        ...
        font-weight: bold;
        @include box_shodow2( 0 .3rem .75rem 0 );
    }
    .btnun {
        width: 10.25rem;
        height: 2.5rem;
...
.pop1-select-btn {
    ...
    @include bg_color6;
    border-radius: 0.2rem;
    padding-top: 0.25rem; /* (40-10-0.5*40)/40 */
    .icon-text {
        font-weight: bold;
        position: absolute;
        left: -0.75rem;
        ...
        height: 1.7rem;
        @include bg_color();
        @include box_shodow4(0 .3rem .75rem 0);
    ...
    .btn-commit {
        ...
        @include bg_color;
        border-radius: 1.25rem;
        margin: 1.6rem /* 64/40 */ auto 0.75rem /* (40-10)/40 */;
        display: flex;
        justify-content: center;
        align-items: center;
        font-size: 0.9rem;
    ...
    .select-btn-add {
        ...
        li {
            ...
            .active{
                @include bg_color6;
                @include font_color16;
                @include border9(0.025rem solid);
            }
            .mix-fonts{
                font-weight: 500;
            }
        ...
        .must-text {
            ...
            margin-bottom: 0.3rem; /* (32-0.5*40)/40 */
            display: flex;
            align-items: center;
            font-weight: 400;
            span {
